...
(function() {
    ...
    function f(b, c, e) {
        ...
        if (b.content) {
            ...
            f.childNodes[b.title ? 1 : 0].appendChild(l)
        }
    ...
    function g(a, d, e) {
        ...
        var h = Math.abs(Math.floor(((window.innerWidth || document.documentElement.offsetWidth) - g.offsetWidth) / 2)),
            l = Math.abs(Math.floor(((window.innerHeight || document.documentElement.offsetHeight) - g.offsetHeight) / 2));
...
scheduler._mouse_coords = function(a) {
    ...
    if (this._table_view) {
        ...
        for (var f = 0, f = 1; f < this._colsS.heights.length; f++)
            if (this._colsS.heights[f] > b.y) break;
        ...
        b.x = 0
    } else b.x = Math.min(this._cols.length - 1, Math.max(0, Math.ceil(b.x / this._cols[0]) - 1)), b.y = Math.max(0, Math.ceil(b.y * 60 / (this.config.time_step *
...
scheduler._on_mouse_move = function(a) {
    if (this._drag_mode) {
        ...
        if (!this._drag_pos || b.force_redraw || this._drag_pos.x != b.x || this._drag_pos.y != b.y) {
            ...
            if (this._drag_mode == "move") c = this._min_date.valueOf() + (b.y * this.config.time_step + b.x * 1440 - (scheduler._move_pos_shift || 0)) * 6E4, !b.custom && this._table_view && (c += this.date.time_part(h.start_date) * 1E3), c = this._correct_shift(c), d = h.end_date.valueOf() - (h.start_date.valueOf() - c);
            else {
                ...
                } else if (d = this.date.date_part(new Date(h.end_date)).valueOf() + b.y * this.config.time_step * 6E4, this._els.dhx_cal_data[0].style.cursor =
                    "s-resize", this._mode == "week" || this._mode == "day") d = this._correct_shift(d);
            ...
            var j = new Date(d - 1),
                m = new Date(c);
            if (this._table_view || j.getDate() == m.getDate() && j.getHours() < this.config.last_hour || scheduler._allow_dnd)
                if (h.start_date = m, h.end_date = new Date(d), this.config.update_render) {
                    var n =
                        scheduler._els.dhx_cal_data[0].scrollTop;
                    this.update_view();
                    scheduler._els.dhx_cal_data[0].scrollTop = n
...
scheduler._on_mouse_down = function(a, b) {
    if (a.button != 2 && !this.config.readonly && !this._drag_mode) {
        ...
        if (this._drag_mode) {
            ...
            !this.config["drag_" + this._drag_mode] || !this.callEvent("onBeforeDrag", [d, this._drag_mode, a]) ? this._drag_mode = this._drag_id = 0 : (this._drag_id = d,
...
scheduler._reset_month_scale = function(a, b, c) {
    ...
    for (var n = "<table cellpadding='0' cellspacing='0'>",
        ...
        for (var p = 0; p < 7; p++) {
            ...
            n += " class='" + q + " " + this.templates.month_date_class(c, e) + "' >";
            var o = "dhx_month_body",
                r = "dhx_month_head";
            p === 0 && this.config.left_border && (o += " dhx_month_body_border", r += " dhx_month_head_border");
            n += "<div class='" + r + "'>" + this.templates.month_day(c) + "</div>";
            n += "<div class='" + o + "' " + g[p] + "></div></td>";
            l.push(c);
    ...
    if (this._table_view)
        if (b) this._colsS.heights = d;
            ...
            if (g.rows) {
                ...
                var o = this._els.dhx_multi_day[0];
                o.style.height = q;
                o.style.visibility = e[0] == -1 ? "hidden" : "visible";
                var r = this._els.dhx_multi_day[1];
                r.style.height = q;
                r.style.visibility = e[0] == -1 ? "hidden" : "visible";
                r.className =
                    e[0] ? "dhx_multi_day_icon" : "dhx_multi_day_icon_small";
                this._dy_shift = (e[0] + 1) * c;
                e[0] = 0;
                if (p != l) f.style.top = parseInt(f.style.top) + 2 + "px", o.style.overflowY = "auto", o.style.width = parseInt(o.style.width) - 2 + "px", r.style.position = "fixed", r.style.top = "", r.style.left = ""
            }
        }
    return a
};
    ...
    for (var e = 0; e < a.length; e++) {
        ...
        if (!b) {
            ...
            for (var j = c[f._sday]; j.length;) {
                var m = j[j.length - 1],
                    n = this._get_event_mapped_end_date(m);
                if (n.valueOf() <= f.start_date.valueOf()) j.splice(j.length - 1, 1);
                else break
            }
            for (var l = !1, p = 0; p < j.length; p++)
                if (m = j[p], n = this._get_event_mapped_end_date(m), n.valueOf() <= f.start_date.valueOf()) {
                    l = !0;
                    f._sorder = m._sorder;
                    j.splice(p, 1);
                    f._inner = !0;
            ...
            if (!l)
                if (j.length)
                    if (j.length <= j[j.length - 1]._sorder) {
                        ...
                        q = j[0]._sorder;
                        for (p = 1; p < j.length; p++)
                            if (j[p]._sorder > q) q = j[p]._sorder;
                        f._sorder = q + 1;
                        f._inner = !1
                    } else f._sorder = 0;
            j.push(f);
        ...
        if (k < this.config.first_hour || i >= this.config.last_hour)
            if (d.push(f), a[e] = f = this._copy_event(f), k < this.config.first_hour && (f.start_date.setHours(this.config.first_hour), f.start_date.setMinutes(0)), i >= this.config.last_hour && (f.end_date.setMinutes(0),
                    f.end_date.setHours(this.config.last_hour)), f.start_date > f.end_date || k == this.config.last_hour) a.splice(e, 1), e--
    }
    if (!b) {
...
scheduler._pre_render_events_table = function(a, b) {
    ...
    for (var c = [], d = [
        ...
        var m = h[j],
            n = f || i.start_date,
            l = i.end_date;
        if (n < this._min_date) m.first_chunk = !1, n = this._min_date;
        if (l > this._max_date) m.last_chunk = !1, l = this._max_date;
        var p = this.locate_holder_day(n, !1, i);
        i._sday = p % g;
        ...
        for (r = 0; r < o.length; r++)
            if (o[r]._eday <= i._sday) break;
        if (!i._sorder || !b) i._sorder = r;
        if (i._sday + i._length <= g) f = null, c.push(i), o[r] = i, e[i._sweek] = o.length - 1, i._first_chunk = m.first_chunk, i._last_chunk = m.last_chunk;
        else {
    ...
    if (!(a._sday < 0)) {
        ...
        if (d) {
            ...
            if (this._edit_id ==
                ...
                this.set_xy(o, j - 6, i - 26);
                o.style.cssText += ";margin:2px 2px 2px 2px;overflow:hidden;";
                q.appendChild(o);
                this._els.dhx_cal_data[0].appendChild(q);
                this._rendered.push(q);
                o.innerHTML = "<textarea class='dhx_cal_editor'>" + a.text + "</textarea>";
                if (this._quirks7) o.firstChild.style.height = i -
                    12 + "px";
                this._editor = o.firstChild;
                ...
                scheduler._focus(o.firstChild, !0);
...
scheduler._render_v_bar = function(a, b, c, d, e, f, g, h, k) {
    ...
    if (!scheduler.renderEvent || k || !scheduler.renderEvent(o, j)) {
        ...
        r += '<div class="dhx_event_move dhx_body" style=" width:' + (d - (this._quirks ? 4 : 14)) + "px; height:" + (e - (this._quirks ? 20 : 30) + 1) + "px;" + l + "" + p + '">' + h + "</div>";
...
scheduler.render_event_bar = function(a) {
    ...
    i && (k = k + " " + i);
    var j = a.color ? "background:" + a.color + ";" : "",
    ...
    for (var e = a.doXPath("//coll_options"), f = 0; f < e.length; f++) {
        ...
        if (h) {
            ...
            for (var k = a.doXPath(".//item", e[f]), i = 0; i < k.length; i++) {
                for (var j = k[i], m = j.attributes, n = {
                        key: k[i].getAttribute("value"),
                        label: k[i].getAttribute("label")
                    }, l = 0; l < m.length; l++) {
                    var p = m[l];
                    if (!(p.nodeName == "value" || p.nodeName == "label")) n[p.nodeName] = p.nodeValue
                }
                h.push(n)
            }
        }
            ...
            if (f.full_day) {
                ...
                i.onclick = function() {
                    if (i.checked) {
                        ...
                        if (+j ==
                            +f || +j >= +f && (c.end_date.getHours() != 0 || c.end_date.getMinutes() != 0)) j = scheduler.date.add(j, 1, "day")
                    }
                    g[h[0]].disabled = i.checked;
                    g[h[0] + g.length / 2].disabled = i.checked;
